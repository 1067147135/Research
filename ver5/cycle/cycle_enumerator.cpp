...
void CycleEnumerator::fast_build_bigraph() {
    ...
    // One cache line contains 64 AVX256 instructions and each AVX256 instruction can manipulate 32 boolean values?
    ...
    for (uint32_t i = 0; i < active_vertices_count; ++i) {
        ...
        // Find the bucket id from buckets_offset_. Only for performance count.
        if (i != 0) {
            while (i >= cur_bucket_offset) {
                cur_bucket_id += 1;
                cur_bucket_max_degree_offset = cur_bucket_id * (length_constraint_ + 1);    // for performance count
...
uint64_t CycleEnumerator::para_dfs(uint32_t ele) {
    // printf("start: thread: %d, ele: %d\n", omp_get_thread_num(), ele);
    uint64_t c = 0;
    std::vector<bool> vis(digraph_->num_vertices_, false); // only include vertices on the path utill now
    vis[src_] = true;
    vis[ele] = true;
    std::vector<node> sta;  // stack to store the vertices to explore next
    sta.push_back({src_, ele, 1});
    std::vector<uint32_t> res(length_constraint_ + 1, -1);  // path from s to t (explored)
    res[0] = src_;
    int p = 0; // pointer to parent layer
    while (!sta.empty()){
        int step = sta.back().step;
        uint32_t v = sta.back().ele;
        uint32_t par = sta.back().parent;
        // std::cout << "visit: parent: " << par << ", element: " << v << ", step: " << step << std::endl;
        if (step > p+1){
        for (;p < step-1;){
            vis[par] = true;
            res[++p] = par;
        ...
        sta.pop_back();
        if (step >= length_constraint_) continue;  

        uint32_t budget = length_constraint_ - step - 1;
        uint32_t neighbor_offset = single_bigraph_[v];
        uint32_t start = single_bigraph_offset_[neighbor_offset];
        uint32_t end = single_bigraph_offset_[neighbor_offset + budget + 1];

        for (uint32_t i = start; i < end; ++i) {
            // printf("dfs: thread: %d, ele: %d\n", omp_get_thread_num(), ele);
            uint32_t w = single_bigraph_adj_[i];
            if (w == dst_) {
                c += 1;
            }
            else if (step == length_constraint_ - 2 && !vis[w]) {
                c += 1;
            }
            else if (!vis[w]) {
                sta.push_back({v ,w, step + 1});
            }
        } 
    }
...
bool cmp(std::pair<int, uint32_t> a, std::pair<int, uint32_t> b){
...
void CycleEnumerator::dfs_on_bigraph(uint32_t u, uint32_t k) {
    ...
    std::vector<std::pair<int, uint32_t>> neigh;
    int j = end - start;
    for (uint32_t i = start; i < end; ++i) {
        uint32_t v = single_bigraph_adj_[i];
        int degree = single_bigraph_offset_[single_bigraph_[v] + budget] - single_bigraph_offset_[single_bigraph_[v]];
        neigh.push_back({degree,v});
    }
    std::sort(neigh.begin(), neigh.end(), cmp);

    neighbors_access_count_ += (end - start);

    omp_set_num_threads(NumThreads);    // firstprivate(Tvisited)
    #pragma omp parallel for reduction(+:count_)  schedule(dynamic)
        ...
        for (int i = 0; i < j; ++i) {
            ...
            uint32_t v = neigh[i].second;
            if (v == dst_) {
                count_ += 1;
            }
            else if (k == length_constraint_ - 2) {
                count_ += 1;
            }
            else {
                count_ += para_dfs(v);
            ...
            // auto dur = std::chrono::duration_cast<std::chrono::nanoseconds>(tp4 - tp3).count();
            // std::cout << "Thread " << omp_get_thread_num() << " finished work, used time: " << dur / (double)1000000000 << " seconds." << std::endl;
