...
int main(int argc, char *argv[]) {
    ...
    for (uint32_t i = 2; i < num_queries; ++i) {
    ...
    uint64_t total_distance_time = std::accumulate(enumerator.distance_time_arr_.begin(), enumerator.distance_time_arr_.end(), 0ull);
    ...
    uint64_t total_count_c = std::accumulate(enumerator.count_c_arr.begin(), enumerator.count_c_arr.end(), 0ull);
    uint64_t total_count_o = std::accumulate(enumerator.count_o_arr.begin(), enumerator.count_o_arr.end(), 0ull);
    ...
    log_info("average distance check time: %.6lf seconds", (double)total_distance_time / num_queries / 1000000000.0);
    ...
    log_info("average vertices met while building X: %.2lf", (double)total_count_c / num_queries);
    log_info("average vertices met while building H: %.2lf", (double)total_count_o / num_queries);
    /**
