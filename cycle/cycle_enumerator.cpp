...
bool g_exit = false;
const uint8_t CycleEnumerator::INF8 = 10;
...
void CycleEnumerator::update_performance_counter() {
    count_c_arr.emplace_back(count_c);
    count_o_arr.emplace_back(count_o);
    result_count_arr_.emplace_back(count_);
    query_time_arr_.emplace_back(query_time_);
    distance_time_arr_.emplace_back(distance_time_);
...
void CycleEnumerator::labeling() {
    ...
    index_in_ = (index_t*)memalign(64, num_vertices * sizeof(index_t));
    index_out_ = (index_t*)memalign(64, num_vertices * sizeof(index_t));
    ...
    // reorder points according to degree
    std::vector<std::pair<uint32_t, uint32_t> > deg(num_vertices);
    for (uint32_t v = 0; v < num_vertices; v++){
        deg[v] = std::make_pair(digraph_->out_degree_[v] + digraph_->in_degree_[v], v);
    }
    std::sort(deg.rbegin(), deg.rend());
    std::vector<bool> usd(num_vertices, false);

    

    // pruned BFSs using normal (old) labels for pruning
    {
        std::vector<std::vector<std::pair<uint32_t, uint8_t>>>
            tmp_idx_in(num_vertices, (std::vector<std::pair<uint32_t, uint8_t>>
            (1, std::make_pair(num_vertices, INF8))));
        std::vector<std::vector<std::pair<uint32_t, uint8_t>>>
            tmp_idx_out(num_vertices, (std::vector<std::pair<uint32_t, uint8_t>>
            (1, std::make_pair(num_vertices, INF8))));
        std::vector<bool> vis_in(num_vertices);
        std::vector<bool> vis_out(num_vertices);
        std::vector<uint32_t> que_in(num_vertices);  // queue
        std::vector<uint32_t> que_out(num_vertices);  // queue
        std::vector<uint8_t> dst_r_in(num_vertices + 1, INF8);  // distance to r
        std::vector<uint8_t> dst_r_out(num_vertices + 1, INF8);  // distance to r

        

        for (uint32_t q = 0; q < num_vertices; q++){    // start from high degree vertices, using old name
            uint32_t r = deg[q].second;
            ...
            uint32_t que_t0_in = 0, que_t1_in = 0, que_h_in = 0;
            uint32_t que_t0_out = 0, que_t1_out = 0, que_h_out = 0;  
            que_in[que_h_in++] = r;
            que_out[que_h_out++] = r;
            vis_in[r] = true;
            vis_out[r] = true;
            ...
            for (uint8_t d = 0; que_t0_in < que_h_in || que_t0_out < que_h_out; ++d) {
                // std::cout << "check point 1: start the "<< unsigned(d) << " th for loop about r" << std::endl;
                if (que_t0_in < que_h_in){  // in-neighbor of r
                    // std::cout << "check point 2: start for loop about r's in-neighbor" << std::endl;
                    for (uint32_t que_i_in = que_t0_in; que_i_in < que_t1_in; ++que_i_in) {
                        uint32_t v = que_in[que_i_in];
                        std::vector<std::pair<uint32_t, uint8_t>>
                        ...
                        if (usd[v]){
                            ...
                            // tmp_idx_v_out.push_back(std::make_pair(num_vertices, INF8));
                            // tmp_idx_r_in.push_back(std::make_pair(num_vertices, INF8));
                            continue;
                        } 
                        
                        uint32_t *neighbors_in = digraph_->in_neighbors(v).first;
                        uint32_t degree_in = digraph_->in_neighbors(v).second;
                        ...
                        // Traverse
                        // std::cout << unsigned(r) << "'s in-neighbor:" << v << ", distance: " << unsigned(d) << std::endl;
                        tmp_idx_v_out.back() = std::make_pair(r, d);  
                        tmp_idx_r_in.back() = std::make_pair(v, d);
                        tmp_idx_v_out.push_back(std::make_pair(num_vertices, INF8));
                        tmp_idx_r_in.push_back(std::make_pair(num_vertices, INF8));
                        ...
                        for (uint32_t i = 0; i < degree_in; ++i) {
                            uint32_t w = neighbors_in[i];
                ...
                if (que_t0_out < que_h_out){
                    // std::cout << "check point 3: start for loop about r's out-neighbor" << std::endl;
                    for (int que_i_out = que_t0_out; que_i_out < que_t1_out; ++que_i_out) {
                        ...
                        if (usd[v]){
                            ...
                            // tmp_idx_v_in.push_back(std::make_pair(num_vertices, INF8));
                            // tmp_idx_r_out.push_back(std::make_pair(num_vertices, INF8));
                            continue;
                        } 
                        
                        uint32_t *neighbors_out = digraph_->out_neighbors(v).first;
                        uint32_t degree_out = digraph_->out_neighbors(v).second;
                        ...
                        // Traverse
                        // std::cout << unsigned(r) << "'s out-neighbor:" << v << ", distance: " << unsigned(d) << std::endl;
                        tmp_idx_v_in.back() = std::make_pair(r, d);  
                        tmp_idx_r_out.back() = std::make_pair(v, d);
                        tmp_idx_v_in.push_back(std::make_pair(num_vertices, INF8));
                        tmp_idx_r_out.push_back(std::make_pair(num_vertices, INF8));
                        ...
                        for (size_t i = 0; i < degree_out; ++i) {
                            int w = neighbors_out[i];
            ...
            // std::cout << "check point 4: r's neighbor enumeration finished" << std::endl;
    ...
    auto end = std::chrono::high_resolution_clock::now();
    auto construct_label_time_ = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
    calculated_memory_cost_ = num_edges;
    index_edge_count_ = num_edges;
    index_vertex_count_ = num_vertices;

    std::cout << "2-hop-labeling finished, using time: " << construct_label_time_ << std::endl;
}

int CycleEnumerator::QueryDistance(int v, int w) {
    auto start = std::chrono::high_resolution_clock::now();
    uint32_t num_vertices = digraph_->num_vertices();
    if (v == w) return 0;
    if (v >= num_vertices || w >= num_vertices) return INF8;  // INT_MAX = 2147483647
    ...
    for (int i1 = 0, i2 = 0; ; ) {
        ...
        if (v1 == v2) {
            if (v1 >= num_vertices) break;  // Sentinel
    ...
    if (d >= INF8 - 2) d = INF8;
    auto end = std::chrono::high_resolution_clock::now();
    distance_time_ += std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
    return d;
}
...
void CycleEnumerator::fast_build_bigraph() {
    ...
    auto out_neighbors = digraph_->out_neighbors(src_);
    for (uint32_t i = 0; i < out_neighbors.second; ++i) {
        uint32_t vv = out_neighbors.first[i];
        if (!visited_[vv]) {
            visited_[vv] = true;         
            if (QueryDistance(vv, dst_) < k){
                distance_[vv].first = 1;
                q.push(vv);
            }
            updated_values_[updated_values_count++] = vv; 
        }
    }
    count_c += out_neighbors.second;
    ...
    while (!q.empty()) {
        ...
        if (distance_[v].first < k - 1) {
            ...
            out_neighbors = digraph_->out_neighbors(v);
            for (uint32_t i = 0; i < out_neighbors.second; ++i) {
                uint32_t vv = out_neighbors.first[i];
...
void
 CycleEnumerator::parallel_dfs(uint32_t u, uint32_t k) {
    ...
    for (int i = 0; i < NumThreads; i++){
        count[i] = 0;
        partial_result_count[i] = 0;
        conflict_count[i] = 0;
        invalid_partial_result_count[i] = 0;
    }
    ...
    for (int i = 0; i < NumThreads; i++){
        ...
        count_ += count[i];
        partial_result_count_ += partial_result_count[i];
        conflict_count_ += conflict_count[i];
        invalid_partial_result_count_ += invalid_partial_result_count[i];
    }
