...
public:
    ...
    std::vector<uint64_t> distance_time_arr_;    // In nanoseconds.
    ...
    std::vector<uint64_t> count_c_arr;
    std::vector<uint64_t> count_o_arr;
    std::vector<double> estimate_accuracy_arr_;

    uint64_t query_time_;
    uint64_t distance_time_;
