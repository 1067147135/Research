...
void CycleEnumerator::para_dfs(uint32_t u, uint32_t k) {
    Tvisited[u] = true;
    ...
    for (uint32_t i = start; i < end; ++i) {
        ...
        else if (k == length_constraint_ - 2 && !Tvisited[v]) {
            // Emit the result.
            // stack_[k + 1] = v;
            // stack_[k + 2] = dst_;
            count_ += 1;
...
void CycleEnumerator::dfs_on_bigraph(uint32_t u, uint32_t k) {
    ...
    std::vector<bool> tmp(digraph_->num_vertices_, false);
    swap(Tvisited, tmp);
    Tvisited[u] = true;
    ...
    // , partial_result_count_, conflict_count_, invalid_partial_result_count_
    #pragma omp for reduction(+:count_) firstprivate(Tvisited) schedule(dynamic)
